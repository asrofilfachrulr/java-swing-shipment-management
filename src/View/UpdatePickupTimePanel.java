...
public class UpdatePickupTimePanel extends NavContentPanel {
	...
	public UpdatePickupTimePanel(MainFrame mainFrame, JPanel prevPanel, DeliveryRequest request) {
		super(mainFrame, prevPanel);
		
		this.request = request;
		
		JLabel lbTitle = new JLabel("Update Pickup Time");
		...
		loadingDialog.showDialogAndRun("Updating", "Saving your changes...", () -> {
			try {
				((Staff) mainFrame.getStore().getAccount())
					...
					.changePickupTime(request.getId(), date);
			} catch (Exception e) {
	...
	@Override
	public void init() {
		Date date = request.getPickupTime();
