...
public class DeliveryRequestDao {
	public void add(DeliveryRequest req) throws SQLException {
		...
		try {
			...
			String insertQuery = "INSERT INTO delivery_requests " + "(sender_name, sender_phone, sender_address, "
					+ "recipient_name, recipient_phone, recipient_address, " + "weight, is_fragile, description, cost, "
	...
	public DeliveryRequest getByDeliveryId(int deliveryId) throws Exception {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		try {
			...
			String query = "SELECT dr.*, c.name AS origin_name, c.metric AS origin_metric, c.name_abbr AS origin_name_abbr, c2.name AS dest_name, c2.metric AS dest_metric, c2.name_abbr AS dest_name_abbr "
					...
					+ "JOIN package_deliveries pd ON dr.id = pd.delivery_req_id "
					+ "WHERE pd.id = ?";
			...
			while (rs.next()) {
				...
				request = new DeliveryRequest(senderName, senderPhone, senderAddress, recipientName,
						recipientPhone, recipientAddress, weight, isFragile, description, cost,
						new Date(requestTime.getTime()), originCity, destCity);
				request.setId(id);
	...
	public List<DeliveryRequest> getAll(int userId) throws Exception {
		...
		try {
			...
			String query = "SELECT dr.*, c.name AS origin_name, c.metric AS origin_metric, c.name_abbr AS origin_name_abbr, c2.name AS dest_name, c2.metric AS dest_metric, c2.name_abbr AS dest_name_abbr "
					+ "FROM delivery_requests dr " + "JOIN cities c ON dr.city_origin_id = c.id "
					+ "JOIN cities c2 ON dr.city_dest_id = c2.id " + "WHERE customer_id = ? "
			...
			while (rs.next()) {
				...
				DeliveryRequest request = new DeliveryRequest(senderName, senderPhone, senderAddress, recipientName,
						recipientPhone, recipientAddress, weight, isFragile, description, cost,
						new Date(requestTime.getTime()), originCity, destCity);
				request.setId(id);
				request.setCustomerId(customerId);
	...
	public List<DeliveryRequest> adminGetAll(int staffId) throws Exception {
		...
		try {
			...
			String query = "SELECT dr.*, c.name AS origin_name, c.metric AS origin_metric, c.name_abbr AS origin_name_abbr, c2.name AS dest_name, c2.metric AS dest_metric, c2.name_abbr AS dest_name_abbr "
					+ "FROM delivery_requests dr " + "JOIN cities c ON dr.city_origin_id = c.id "
					+ "JOIN cities c2 ON dr.city_dest_id = c2.id "
					+ "JOIN staff s ON dr.city_origin_id = s.office_city_id " + "WHERE s.id = ? "
					+ "ORDER BY dr.request_time DESC;";
